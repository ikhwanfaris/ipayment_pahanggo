...
class _ViewEnquiryScreenState extends State<ViewEnquiryScreen> {
  ...
  List<ReassignRemark> reassignRemark = [];
  ...
  void _getData() async {
    ...
    reassignRemark = await api.getReassignRemarkEnquiry(widget.enquiry.id);
    _fileSettingModel = await api.getFileSettingEnquiry();

    setState(() {
      messages = messages;
      reassignRemark = reassignRemark;
    });
                              ...
                              children: [
                                 Text(
                                  updatedAtTime == null
                                      ...
                                      :
                                  DateFormat('dd/MM/yyyy, hh:mm a')
                                      .format(updatedAtTime!), style: styles.heading14Primary,
                                ),
                                Text(
                                ...
                                Align(
                                  alignment:
                                      messages[index].user_id == state.user.id
                                          ? Alignment.centerRight
                                          : Alignment.centerLeft,
                                  child: Text(messages[index].role?.name.toString() ?? 'User'.tr ,style: styles.descHome)),
                                SizedBox(height: 5),
                                Align(
                                  alignment:
                                      messages[index].user_id == state.user.id
                                ...
                                Align(
                                  ...
                                  child: InkWell(
                                    onTap: (() => messages[index]
                                        ...
                                        : null),
                                    child: messages[index].file.toString() ==
                                            'null'
                                        ? SizedBox.shrink()
                                        : Text(
                ...
                child: Padding(
                  ...
                  child: Align(
                      ...
                      child: Text(
                          'Reassign'.tr +
                              ' (' +
                              reassignRemark.length.toString() +
                              ') :',
                          style: styles.headingTitleboldWhite)),
                ),
              ),
              // List reassign messages
              Container(
                child: ListView.builder(
                  itemCount: reassignRemark.length,
                  shrinkWrap: true,
                  physics: BouncingScrollPhysics(),
                  itemBuilder: (context, index) {
                    // final starttime = DateTime.parse(
                    //         reassignRemark[index].created_at.toString())
                    //     .toLocal();
                    return Padding(
                      padding: const EdgeInsets.all(12),
                      child: Container(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                   Text(
                                    ...
                                    SizedBox(height: 5),
                                    Text(
                                      reassignRemark[index].user!.first_name! +
                                          ' ' +
                                          reassignRemark[index]
                                              .user!
                                              .last_name!,
                                      style: styles.heading5bold,
                                    ),
                                    SizedBox(height: 5),
                                    Text(reassignRemark[index].role!.name.toString()),
                                    SizedBox(height: 5),
                                    Text(reassignRemark[index].remark.toString()),
                                    SizedBox(height: 5),
                                    InkWell(
                                      onTap: (() => reassignRemark[index]
                                                  .file
                                                  .toString() !=
                                              'null'
                                          ? launchUrl(Uri.parse(endpoint +
                                              '/file/' +
                                              reassignRemark[index].file.toString()))
                                          : null),
                                      child: reassignRemark[index]
                                                  .file
                                                  .toString() ==
                                              'null'
                                          ? SizedBox.shrink()
